...
/*   Updated: 2021/11/05 15:05:30 by acoezard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_vector2	*ft_projection(t_vector3 *vector, t_fdf *fdf)
{
	...
	camera = fdf->camera;
	map = fdf->map;
	x = vector->x * camera->zoom;
	y = vector->y * camera->zoom;
	...
	x = camera->x + (save->x - save->y) * cos(FT_PI / 4);
	y = camera->y + (save->x + save->y) * sin(FT_PI / 4) - save->z;
	free(save);
